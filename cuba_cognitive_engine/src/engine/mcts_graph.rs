...
impl<'a> MctsGraph<'a> {
    ...
    pub fn calculate_puct(&self, node_id: usize) -> Result<f64, &'static str> {
        let node = self
            .nodes
            .get(node_id)
            .ok_or("MCTS: node_id out of bounds")?;
...
mod tests {
    ...
    fn test_arena_allocation_and_puct_math() {
        ...
        let puct_score = mcts
            .calculate_puct(1)
            .expect("PUCT should succeed for valid node");

        // Use an epsilon for float comparison
        assert!(
            (puct_score - 1.327046).abs() < 0.001,
            "PUCT mathematical failure: {}",
            puct_score
        );

        // P1-1: Verify OOB returns error instead of panic
        assert!(
            mcts.calculate_puct(999).is_err(),
            "OOB node_id should return Err"
        );
